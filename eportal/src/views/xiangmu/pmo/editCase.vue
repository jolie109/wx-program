      ...
      <v-layout justify-center>
        <v-dialog v-model="dialog" persistent max-width="700">
          <v-card>
            ...
            <v-layout>
              <v-flex lg4 class="row_lab_sty">
                <v-subheader>结案分数</v-subheader>
              </v-flex>
              <v-flex xs6>
                <v-text-field v-model="pro_lineData.score"></v-text-field>
              </v-flex>
            </v-layout>
            <v-layout>
              <v-flex lg4 class="row_lab_sty">
                <v-subheader>项目结案状态</v-subheader>
              </v-flex>
              <v-flex xs6>
                <v-select :items="items" v-model="pro_lineData.status"></v-select>
              </v-flex>
            </v-layout>
...
export default {
  data() {
    return {
      ...
      items: ["已结案（成功）", "已结案（失败）"],
      category_items: ["Collaboration", "Management"],
      issueTimePoint_items: ["已结案", "项目执行中"]
    };
  },
  mounted() {
    // 编辑按钮是否显示
      ...
      this.$axios
        ...
        .then(res => {
          (this.desserts[0].name = this.pro_lineData.name),
            (this.desserts[0].score = this.pro_lineData.score),
            (this.desserts[0].status = this.pro_lineData.status);
