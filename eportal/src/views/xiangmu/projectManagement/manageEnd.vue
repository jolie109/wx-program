<template>
  <div class="container">
        ...
        >
          <template v-slot:items="props">
            ...
            <tr>
              ...
              <td>{{ props.item.complete_pm_point+props.item.reward_pm_point }}</td>
    ...
    <v-layout justify-center>
      <v-dialog v-model="dialogApplication" persistent max-width="900px">
        <v-card>
          <v-flex md12 d-flex>
              ...
              >
                ...
                <template v-slot:footer>
                  ...
                  <td class="text-md-center">
                    <v-btn small color="primary" @click="submitApplication()">提交申请</v-btn>
                  </td>
                </template>
              </v-data-table>
    ...
    <v-snackbar color="error" top right :timeout="timeout0" v-model="snackbar0">
      ...
      <div>{{error}}</div>
      <v-icon color="white" size="16" @click="snackbar2 = false">clear</v-icon>
    </v-snackbar>
     <!-- 用于点数提示的snackbar -->
     <!-- 弹框 -->
  ...
  data() {
    return {
      ...
      headers_medal: [
        ...
        { text: "可发放总额-执行中", align: "left", sortable: false },
        { text: "已发放总额-执行中", align: "left", sortable: false },
        { text: "可发放总额-结案", align: "left", sortable: false },
        { text: "已发放总额-结案", align: "left", sortable: false },
        { text: "发放角色", align: "left", sortable: false }
      ],
      desserts_medal: [],
      headers_dianForm: [
        { text: "项目成员", align: "left", sortable: false },
                    ...
                    this.pmoalreadysend=true
                     ...
                     this.pmoalreadysend=false
                       res.data.result.forEach(item=>{
                         ...
                         {  //包括了结案和奖励
                           this.pmosendothercc+=Number(item.cc_point)
                           this.pmosendotherpm+=Number(item.pm_point)
                         }
          ...
          if(this.projectStatus=="项目执行中"){
            if(i.point_type=="Collaboration"){
             ...
             if(i.point_type=="Collaboration"){
              ...
              if((Number(PMsendcc)+Number(PMalreadysendothercc))>this.pmosendothercc){
                // alert("结案超出cc合计")
