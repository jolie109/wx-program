                    ...
                    <v-card v-show="addShow">
                        ...
                        <v-card-text>
                            <v-container grid-list-md>
                                <v-layout>
                                    <span class=baseTitle>路由名称</span>
                                    <v-flex xs12 sm6 md4>
                                        <v-text-field label="输入路由名称" v-model="editedItem.name"></v-text-field>
                                    </v-flex>
                                </v-layout>
                                  <v-layout>
                                    <span class=baseTitle>Path</span>
                                    <v-flex xs12 sm6 md4>
                                        <v-text-field label="输入路径" v-model="editedItem.path"></v-text-field>
                                    </v-flex>
                                </v-layout>                   
                            </v-container>
                        </v-card-text>
...
export default {
    data() {
        return {
            ...
            {text: '路由名称',sortable: false,value: 'name'},
             {text: '路径',sortable: false,value: 'name'},
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? '增加路由' : '编辑内容'
      }
    },
    mounted(){
        this.getList();
    ...
    methods:{
        ...
        handleDelSave(){
            this.$axios.delete('/api/admin/acls',{
                data:{
                    _id:this.categoryId 
                }
            }).then((res) => {
        ...
        save(){
            ...
            if(this.editedIndex>-1){
                this.$axios.post('/api/admin/updateAcl',{
                ...
                this.$axios.post('/api/admin/acls',{
                    data:this.editedItem
                }).then((res) => {
