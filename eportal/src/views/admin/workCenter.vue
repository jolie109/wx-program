    ...
    <div>
                    ...
                    <v-card v-show="addShow">
                        ...
                        <v-card-text>
                            <v-container grid-list-md>
                                <v-layout>
                                    <span class=baseTitle>中心名称</span>
                                    <v-flex xs12 sm6 md4>
                                        <v-text-field label="输入打印中心名称" v-model="editedItem.workName"></v-text-field>
                                    </v-flex>
                                </v-layout>                  
                            </v-container>
                        </v-card-text>
        ...
        <div class="classifyBg">
            ...
            >
                ...
                <td class="text-xs-left" >{{ props.item.workName}}</td>
...
export default {
    data() {
        return {
            ...
            headers: [
            {text: 'uuId',sortable: false,value: 'name'},
            {text: '中心名称',sortable: false,value: 'name'},
    ...
    computed: {
      formTitle () {
        return this.editedIndex === -1 ? '增加中心' : '编辑内容'
      }
    },
    mounted(){
        this.getList();
    ...
    methods:{
        ...
        handleDelSave(){
            this.$axios.delete('/api/admin/workCenters',{
                data:{
                    _id:this.categoryId 
                }
            }).then((res) => {
        ...
        save(){
            ...
            if(this.editedIndex>-1){
                this.$axios.post('/api/admin/updateworkCenters',{
                ...
                this.$axios.post('/api/admin/workCenters',{
                    data:this.editedItem
                }).then((res) => {
