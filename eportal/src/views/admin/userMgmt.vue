                        ...
                        <v-card-text>
                            <v-container grid-list-md>
                                <v-layout>
                                  ...
                                  <v-layout>
                                    <span class=baseTitle>手机号</span>
                                    <v-flex xs12 sm6 md4>
                                        <v-text-field label="输入手机号码" v-model="editedItem.tel"></v-text-field>
                                    </v-flex>
                                </v-layout>
                                      <v-layout wrap>
                                    <span class=baseTitle>角色权限</span>
                                    <v-flex xs12 sm6 md4>
                                        <v-select
                                         item-text="name"
                                        :items="roleList"
                                 ...
                                 <v-layout wrap>
                                    <span class=baseTitle>打印中心权限</span>
                                    <v-flex xs12 sm6 md4>
                                        <v-autocomplete
                                         item-text="workName"
                                        :items="centerList"
                                        item-value="_id"
                                        v-model="editedItem.item2"
                                        label="请选择"
                                        required
                                        multiple
                                        ></v-autocomplete>
            ...
            >
                ...
                <td class="text-xs-left">
                  ...
                  {{roles.name}} &nbsp;&nbsp;
                    ...
                    <td class="text-xs-left">
                  <template v-for="(centers) in props.item.item2">
                  {{centers.workName}} &nbsp;&nbsp;
                  </template>
                </td>
...
export default {
        ...
        return {
          ...
          centerList:[],
            ...
            {text: '用户',sortable: false,value: 'name'},
             ...
             {text: '角色权限',sortable: false,value: 'name'},
             {text: '打印中心权限',sortable: false,value: 'name'},
    ...
    methods:{
        ...
        getRoles(){
            this.$axios.get('/api/admin/userRoles').then((res) => {
                this.roleList=res.data.result;
            })
            
        },
        getCenters(){
            this.$axios.get('/api/admin/workCenters').then((res) => {
        ...
        handleDelSave(){
            this.$axios.delete('/api/admin/users',{
                data:{
                    _id:this.userId 
                }
            }).then((res) => {
        ...
        save(){
            ...
            if(this.editedIndex>-1){
                this.$axios.post('/api/admin/updateUser',{
