      ...
      <div class="lowBg">
        ...
        <v-flex md12>
            ...
            <template v-slot:items="props">
              ...
              <td class="text-xs-center">
                <v-btn
                  ...
                  small
                  color="primary"
                  @click="deleteAdjustItem(props.item)"
                >删除</v-btn>
              </td>
            </template>
          </v-data-table>
          ...
          <v-layout style="padding:50px 10px 40px 0;">
            <v-spacer></v-spacer>
            <v-btn v-if="status=='草稿'?true:false" small color="primary" @click="saveAll()">保存</v-btn>
            <v-btn small color="primary" @click="cancelAll()">{{ status=="草稿"?"取消":"返回" }}</v-btn>
            <v-btn v-if="status=='草稿'?true:false" small color="primary" @click="publishAll()">发布</v-btn>
          </v-layout>
        </v-flex>
      </div>
    </div>
...
export default {
  ...
  data: () => ({
    ...
    dessertfifth: [],
    rewardHeader: [
      { text: "内容", sortable: false, value: "contain", align: "center" },
      { text: "金额", sortable: false, value: "outTime", align: "center" },
      {
        text: "操作",
        sortable: false,
        align: "center"
  ...
  methods: {
    addItem(item) {
      this.dialog = true;
      this.editIndex = this.rewardList.indexOf(item);
    ...
    addAdjustItem(item) {
      this.dialog = true;
      this.editIndex = this.adjustList.indexOf(item);
      this.editedAdjustItem = Object.assign({}, item);
      this.adjustShow = true;
      this.rewardShow = false;
    },
    deleteItem(item) {
      const index = this.rewardList.indexOf(item);
       this.rewardList.splice(index,1)
     
    },
    deleteAdjustItem(item) {
      const index = this.adjustList.indexOf(item);
    ...
    saveReward() {
      ...
      this.rewardList.push(this.rewardAdd);
      this.dialog = false;
    },
    //调整弹框点击创建
    saveAdjust() {
      ...
      this.adjustList.push(this.adjustAdd);
      this.dialog = false;
    },
    getPayslipDetail() {
      this.$axios
        .get("/api/salaryList/payslipDetailEdit", {
          params: {
        ...
        .then(res => {
          ...
          switch (res.data.status) {
            case 200:
              var rewardListStep = [];
              var adjustListStep = [];
              rewardListStep = res.data.result[0].monthly_payslip.special_adjust.filter(
                item => {
                  return item.adjust_type == "嘉奖";
                }
              );
              adjustListStep = res.data.result[0].monthly_payslip.special_adjust.filter(
                item => {
                  return item.adjust_type == "调整";
                }
              );
              this.rewardList = rewardListStep;
              this.adjustList = adjustListStep;
    ...
    saveAll() {
      this.status = "草稿";
      this.special_adjust = [...this.rewardList, ...this.adjustList];
      this.$axios
        .put("/api/salaryList/editPayslip", {
    ...
    go(){
      ...
      this.strdata.forEach(item => {
       ...
       this.summoney=(this.addedSum+ this.basicSum+ this.adddata+this.reward-this.adjustdata +this.totalAmount).toFixed(2)
   }
  },
  watch:{
     rewardList:function(newval,oldval){
      this. adddata=0
       if(newval.length>0){
      ...
      deep: true,
    adjustList:function(newval,oldval){
       this.adjustdata=0;
      this.reward=0;
      if(newval.length>0){
         newval.forEach(item1=>{
            ...
            }else if(item1.amount&&item1.amount.indexOf("-")==-1) {
...
.headerTitle span:nth-of-type(1) {
  position: absolute;
  top: 45px;
  right: 120px;
}
