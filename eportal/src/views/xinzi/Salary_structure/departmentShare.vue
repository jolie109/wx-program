      ...
      <v-dialog v-model="dialog" max-width="500px">
        <v-card justify-center>
          ...
          <v-card-actions>
            ...
            <v-btn color="info" flat :disabled="btn_submit" @click="saveEdit()">确定</v-btn>
...
export default {
  ...
  data: () => ({
    ...
    defaultItem: {},
    headers: [
      { text: "分红类型", align: "center", value: "type", sortable: false },
      { text: "所属周期", align: "center", value: "week", sortable: false },
    ...
    show_btn_2: false,
    show_btn_3: false,
    departnormalshareAmount:0,
     departmentid: "",
     departAllAmount:0
  }),
  computed: {},
  created() {
    this.getBonusDepartShare();
    this.initialize();
    // 股权分红时不显示编辑按钮
    if (this.$route.query.show_edit_1 == "unshow") {
      this.show_edit = false;
      this.headers.pop();
    } else if (this.$route.query.show_edit_2 == "unshow") {
      this.show_edit = false;
    }
  },
  methods: {
    ...
    getBonusDepartShare() {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          this.topAmount =
            this.desserts[0].origin_amount + this.desserts[0].origin_amount * 0.2;
          this.bottomAmount =
            this.desserts[0].origin_amount - this.desserts[0].origin_amount * 0.2;
        });
    },
      ...
      if (a > this.bottomAmount && a < this.topAmount) {
        this.$axios
          ...
          .then(res => {
            ...
            //部门的一般分红
              this.departnormalshareAmount =
               +( this.departmentinfo.departNormalPerforShare + this.departmentinfo.departNormalMedalShare).toFixed(2);
            //调整后的部门合计
              this.$axios
                ...
                .then(res => {
                  ...
                  let adjustamount=afterdepartAllAmount-this.departnormalshareAmount //部门调整前后的差值
                  if (
                  this.departnormalshareAmount<afterdepartAllAmount
                  ) {
                    this.snackbar = true
                    this.color = "error"
                    this.right = true
        ...
        this.color = "error"
        this.right = true
        this.titleTip = "输入分红金额须在" + this.bottomAmount.toFixed(2) + " ~ " + this.topAmount.toFixed(2) + "之间"
        this.btn_submit = true;
        this.ok = null;
      }
    },
